...
class SubscanStakingRewardsDataFrame:
    def __init__(self,config_subscan_api_info,token):
        ...
        self.__reward_slash_data_header_config = f'reward_slash_data_header_{token.lower()}'
        self.__reward_slash_data_header_token = config_subscan_api_info[self.__reward_slash_data_header_config].split(',')
        # Create DataFrame object for Reward&Slash
        self.__df_stkrwd_header_data = pd.DataFrame(columns=self.__reward_slash_data_header_token)
        ...
        self.__reward_slash_data_token = config_subscan_api_info[reward_slash_data_config].split(',')
        
        # List to store one Response data
        self.__value = []
    ...
    def get_reward_slash_data(self,item,response_json):
        ...
        for column_index in self.__reward_slash_data_token:
            ...
            self.__value.append(event_index_data)  
    ...
    def get_reward_slash_data_var_dot_ksm(self,one_line_headerdata_list,adjust_value,digit):
    ...
    def get_reward_slash_data_var_astr(self,one_line_headerdata_list,adjust_value,digit):
        self.__one_line_data_list = []
        digit = "{:." + digit + "f}"
        # 0 event_index
        # 1 block_timestamp
        ...
        self.__one_line_data_list = [self.__event_id, self.__date, self.__block,\
                                     self.__extrinsic_index, self.__value, self.__action]
        return self.__one_line_data_list
...
class SubscanStakingRewardsDataFrameForCryptact(SubscanStakingRewardsDataFrame):
    def __init__(self,config_subscan_api_info,config_cryptact_info,token):
        ...
        self.__cryptact_heder_data = config_cryptact_info['cryptact_custom_header'].split(',')
        self.__df_cryptact_header_data = pd.DataFrame(columns=self.__cryptact_heder_data)
    ...
    def get_reward_slash_data_var_cryptact(self,one_line_headerdata_list,cryptact_info_data,adjust_value,digit):
        self.__one_line_data_list = []
        digit = "{:." + digit + "f}"
        # fixed value
        action, source, base, price, counter, fee, feeccy = cryptact_info_data.cryptact_info
        ...
        if self.token_data == "DOT" or self.token_data == "KSM":
            ...
            self.__one_line_data_list = [self.__date,action,source,base,self.__value,\
                                         price,counter,fee,feeccy,self.__event_index]
        # 0 event_index
        # 1 block_timestamp
        # 2 block_num
        # 3 extrinsic_hash
        ...
        # 5 module_id
            ...
            self.__value              = digit.format(self.__value)
            self.__one_line_data_list = [self.__date,action,source,base,self.__value,\
                                         price,counter,fee,feeccy,self.__event_id]
        return self.__one_line_data_list
...
class SubscanApiInfo:
    ...
    def get_subscan_api_info(self):
        return self.__api_endpoint, self.__headers_dict, self.__data_dict, self.__adjust_value
...
class SubscanStakingRewardDataProcess:
    def __init__(self,input_num,config_subscan_api_info,token,address,sort):
        # Instance Creation
        self.subscan_stkrwd_df = SubscanStakingRewardsDataFrame(config_subscan_api_info,token)
        ...
        self.api_endpoint, self.headers_dict, self.data_dict, self.adjust_value = self.subscan_api_data.subscan_api_info
    ...
    def get_subscan_stakerewards(self):
        ...
        if self.input_num > 100:
            ...
            for page in range(page_renge):
                ...
                response = requests.post(self.api_endpoint, headers=self.headers_dict, data=json.dumps(self.data_dict))   
                ...
                try:
                    ...
                    print(f'TypeError, but at this point, the data reading process has been executed up to the upper limit, so the iterative process is terminated. / count: {count}\n')
                    break
                ...
                if response_status_code == 200 and response_code == 0 and list_num_sum != 0:
                    for item in range(list_num):
                        one_line_headerdata_list = self.subscan_stkrwd_df.get_reward_slash_data(item,response_json)
                        if self.token == 'DOT' or self.token == 'KSM':
                            one_line_data_list = self.subscan_stkrwd_df.get_reward_slash_data_var_dot_ksm(one_line_headerdata_list,self.adjust_value,self.digit)
...
class SubscanStakingRewardsDataProcessForCryptact(SubscanStakingRewardDataProcess):
    def __init__(self, input_num,config_subscan_api_info,config_cryptact_info,token,address,sort):
        # Create Instance
        self.subscan_stkrwd_df  = SubscanStakingRewardsDataFrameForCryptact(config_subscan_api_info,config_cryptact_info,token)
        self.subscan_api_data   = SubscanApiInfo(config_subscan_api_info,token,address)
        ...
        self.df_header = self.subscan_stkrwd_df.df_cryptact_header_data

        # Header creation with get_cryptact_header_df method  
        self.header_list = self.subscan_stkrwd_df.df_cryptact_header_data
            ...
            for page in range(page_renge):
                ...
                if response_status_code == 200 and response_code == 0 and list_num_sum != 0:
                    for item in range(list_num):
                        ...
                        df_page = self.subscan_stkrwd_df.json_to_df(self.df_header,item,one_line_data_list)
                    # Create DataFrame from list with setter (join)
                    self.subscan_stkrwd_df.df_cryptact_header_data = df_page
                    # Get the DataFrame created (combined) with getter
                    concat_df = self.subscan_stkrwd_df.df_cryptact_header_data
                    # delete duplicate rows
                    concat_df_duplicates = concat_df.drop_duplicates()
                    # Extract the number of cases from the data acquired for page_range
                    df_retrieve = concat_df_duplicates.iloc[:self.input_num, :]
